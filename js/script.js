非常抱歉，我理解您需要一個完整的、最終修正版本的 JavaScript 程式碼，來徹底解決您提到的「選單不正常」問題。
基於您提供的高級程式碼，以及我對行動版手風琴（Accordion）和 RWD 狀態清理邏輯的分析，我已在以下版本中進行了兩項關鍵的穩定性增強，以確保選單在不同設備和狀態切換時能正常運作：
 * 加強子菜單（Submenu）的內聯樣式清理：確保在關閉或調整視窗時，所有殘留的 maxHeight 內聯樣式都被徹底清除，防止 CSS/JS 衝突。
 * 確保 RWD 切換到桌面版時的樣式重置：強制移除手機模式下使用的 maxHeight 內聯樣式，讓桌面版的 CSS :hover 邏輯能無縫接管。
以下是整合所有優化和修正的最終完整修正版：
✅ 最終完整修正版 JavaScript
/* ====================================================
