...
(function() {
    ...
    const onTransitionEndCleanup = (contentElement) => {
        ...
        const handleTransitionEnd = (e) => {
            ...
            // 檢查父層是否為展開狀態（如果 DOM 結構有變動，這是最可靠的判斷）
    ...
    const closeAllMobileSubmenus = () => {
        if (mainNav) {
            ...
            Array.from(mainNav.querySelectorAll('li.dropdown.active')).forEach(li => {
                const submenu = /** @type {HTMLElement | null} */ (li.querySelector('.submenu-container, .submenu')); 
    ...
    const closeMainMenu = () => {
        if (mainNav && mainNav.classList.contains('active')) {
            ...
            body.classList.remove('no-scroll'); // 清理 body 鎖定
    ...
    const handleResizeCleanup = (fitAllFunction) => {
         ...
         if (!isMobileView()) {
             ...
             mainNav?.querySelectorAll('.dropdown').forEach(dropdown => {
                 // 清理桌面模式下不需要的內聯樣式
                 const submenu = /** @type {HTMLElement | null} */ (dropdown.querySelector('.submenu-container, .submenu')); 
                 if (submenu) {
    ...
    const setupAccordion = () => {
        document.querySelectorAll('.accordion-item').forEach((item, index) => {
             ...
             if (headerElement && content) {
                 ...
                 // 初始展開時移除 overflow，允許內容顯示陰影等 (交由 onTransitionEndCleanup 處理)
