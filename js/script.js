...
/**
 * ====================================================================
 * 祥安生命全站核心腳本 (SA Life Integrated) - 2026 旗艦對接版
 * 完美配合：index.html 導覽結構 與 13.2/13.3 CSS 動畫工具類
 * ====================================================================
 */
...
(function () {
    ...
    const initNavigation = () => {
        ...
        // 1. 捲動效果：改變 Header 背景 (Scrolled 類別)
        ...
        const toggleMenu = (forceState) => {
            const isActive = forceState !== undefined ? forceState : !mainNav.classList.contains('active');
            ...
            link.addEventListener('click', (e) => {
                if (window.innerWidth <= CONFIG.MOBILE_BREAKPOINT) {
                    ...
                    if (submenu) {
                        ...
                        // 動態高度過渡
                        const innerList = submenu.querySelector('.submenu');
                        submenu.style.maxHeight = isOpen ? (innerList.scrollHeight + 50) + "px" : "0";
                    }
                }
            });
        });
    ...
    /** B. 滾動動畫對接 (對接 CSS .animate-on-scroll) */
    const initScrollAnimations = () => {
        ...
        // 監視所有標記為動畫的元素與方案卡片
        const animElements = document.querySelectorAll('.animate-on-scroll, .plan-card, .footer-section');
        animElements.forEach(el => {
            // 加入 CSS 強制硬體加速類別
            el.classList.add('transform-layer');
            observer.observe(el);
        });
    };

    /** C. 方案篩選 (對接 plans.html 分頁) */
        ...
        tabBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                planCards.forEach(card => {
                    ...
                    setTimeout(() => {
                        const isMatch = (target === 'all' || card.getAttribute('data-category') === target);
    ...
    const updateDynamicContent = () => {
        ...
        // 對應 HTML 中所有 class 包含 current-year 的元素
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // 勞保計算器與對年計算 (若頁面有相關 ID 則自動初始化)
        if (typeof window.SALife.setupDuinianCalculator === 'function') {
            window.SALife.setupDuinianCalculator();
        }

        console.log("%c祥安生命 系統啟動中...", "color: #ce9d4a; font-weight: bold; font-size: 1.2rem;");
        console.log("導航同步 [OK] | 動畫引擎 [OK] | 2026 環境配置 [OK]");
    });
